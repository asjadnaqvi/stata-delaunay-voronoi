...
{p 4 4 2}
The Delaunay and Voronoi core routines are based on {browse "https://github.com/mapbox/delaunator":Mapbox} and {browse "https://github.com/d3/d3-delaunay":d3} implementations respectively. The S-hull is sweep-hull algorithm that iterates across 
radially-sorted points and iterates the triangle generation as it moves along. It is exceptionally fast as compared to {browse "https://github.com/mapbox/delaunator#performance":other implementations}.
...
{p2coldent : {opt delaunay y x}}The command requires an {it:(y,x)} coordinate pair stored as two numeric variables. Please make sure that the order of the variables is correct. Running the command will automatically add the {it:_ID} variable to the dataset.{p_end}

{p2coldent : {opt res:cale}} Delaunay triangles and Voronoi tessellations are not agnostic to the scale of the axes. They are designed for physical geometry where the relation of the {it:x} and {it:y} scales matter. If we are working with data where one variable is several times the magnitude of the second, then the command will correctly execute the triangles but they might be stretched in one direction. The {it:rescale} option normalizes both the {it:x} and {it:y} variables on the [0,1] range, 
...
{p2coldent : {opt hull}}Exports the convex hull back to Stata as three new variables {it:hull_ID, hull_X, hull_Y}. {it:hull_id} corresponds to the point {it:_ID}. The remaining two variables are the coordinates. They starting point is repeated in order to draw a fully enclosed hull.{p_end}
...
The {it:polygons} option generates a {it:_vorpoly.dta} file, which contains three variables {it:_ID}, {it:_X}, {it:_Y}. This file is essentially a shapefile can be used with the main file using the {stata help spmap:spmap} command.{p_end}
...
{p2coldent : {opt replace}}This option replaces the existing files and hull variables. If {cmd replace} is not specified, and the files already exist, the command will throw an error. This ensures that files are not accidentally overwritten.{p_end}

{p2coldent : {opt addbox}}This is an experimental command that is used for adding a bounding box for triangles. This is still being tested and it is highly recommended to use this only with triangles. It might result errors with Voronoi exports.{p_end}
...
- {bf:1.2} : Triangles and Voronoi are exported as files. All reshaping taken out. The commands are now much faster. This stable release brings the package out of beta.
...
{p 4 4 2}
The package greatly benefitted from discussions with Eric Melse and William Buchanan.
